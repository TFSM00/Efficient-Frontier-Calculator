...
def TB3MS_Data():
    """
    Returns some statistics for the 30 year US Treasury Bill used for estimation of the risk free rate of return
    """
    ...
    with open("treasurybill.csv",'wb') as f: # writes the data to a csv file
        f.write(r.content)
        f.close()

    tb3ms_data = pd.read_csv("treasurybill.csv") # csv data to dataframe
    tb3ms_perc = tb3ms_data["TB3MS"].tolist() # bill rate to list

    tb3ms = []
    effective_monthly_rate = []

    for i in range(0, len(tb3ms_perc)): #gets t-bill rates as decimal percentage
        tb3ms.append(round(float(tb3ms_perc[i]/100),ndigits=4))

    for i in range(0, len(tb3ms)): # calculates the effective monthly rate for each date
...
def TB3MS_RiskFree():
    """
    Returns the risk free rate as a list
    """
